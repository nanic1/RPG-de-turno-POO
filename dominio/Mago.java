...
public class Mago extends Personagem {
    ...
    public void atacar(Entidade alvo) {
        ...
        System.out.printf("%s lanca um feitico em " + alvo.getNome() + " causando %d de dano!", nome, dano);
    }
    ...
    public void usarHabilidade(Habilidade h, Entidade alvo) {
        if (h.getTipo() == TipoHabilidade.CURA) {
            curar(h.getPoder());
            System.out.printf("%s se cura em " + h.getPoder() + " pontos!", nome);
        } else {
            alvo.receberDano(h.getPoder());
            System.out.printf("%s conjura " + h.getNome() + " e causa " + h.getPoder() + " de dano!", nome);
        }
    }
}